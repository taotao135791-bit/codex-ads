...
Keep campaign level, bid, and budget in separate structured sections. Use only
declared current and recommended numbers. If evidence shows a target is too
aggressive, adjust only the target. If evidence shows budget is constrained,
adjust only budget. Keep both stable during a level change unless an explicitly
confirmed emergency requires otherwise.

Never derive a tCPA or tROAS number from an AC label. When no account-specific
number exists, return direction-only guidance or `null`.
