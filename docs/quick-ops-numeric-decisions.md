...
6. 普通运营只选择一个数值变量；
7. 根据权限把理想建议转换成当前可执行动作或客户请求；
8. 输出一张只显示唯一执行建议的短卡片，并在 JSON 中保留计算证据。
...
- 预算消耗率使用多日平均花费除以当前日预算。低于 `0.75` 标记为
  ...
  `BUDGET_CONSTRAINED`。当前预算超过明确业务上限时，
  `BUSINESS_BUDGET_CAP` 优先于普通目标调整。当前 tCPA 已高于业务 CPA 上限，
  或当前 tROAS 已低于业务 ROAS 下限时，也会直接纠正到声明边界；这种硬边界
  纠正不依赖效果成熟度或价值对账，且不会把越界旧值作为回滚值。
- 事件量必须满足账户声明的最低日成熟事件量。变异系数高于 `0.50`，或零事件日超过
  `20%`，会标记为量够但波动。
...
候选值位于当前值和业务边界之间。系统根据 `goal.optimization_priority` 的
...
真实数值回放继续只放在被 Git 忽略的 `workspaces/<project>/replays/`。带数值真值的
快照会单独汇总方向正确率、幅度误差、不安全建议率、正确 no-action 率、回滚率、
人工接受率和污染率；旧五/六文件 Replay 仍可读取。人工没有接受或执行的建议、
以及 `confounded` 样本，不会进入执行后方向/幅度的效果分母。
